...
func TestRunBatchSingleDevice(t *testing.T) {
	...
	runBatch(newRand(), []func(float64) float64{fn}, sink, []string{"dev"}, time.Now().Unix(), 3, 60, 0)
...
func TestRunBatchFleet(t *testing.T) {
	...
	runBatch(newRand(), fns, sink, devices, time.Now().Unix(), 4, 60, 0)
...
func TestRunRealtimeFleet(t *testing.T) {
	...
	runRealtime(context.Background(), newRand(), fns, sink, devices, 2, 1, 0) // 2 points per device, 1 s step
...
func TestRunRealtimeCancellation(t *testing.T) {
	...
	cancel()
	runRealtime(ctx, newRand(), fns, sink, devices, 100, 10, 0)
...
func TestRunRealtimeMultiEmitsPoints(t *testing.T) {
	...
	runRealtimeMulti(context.Background(), newRand(), fieldFns, scales, 0, 0, 0, 0, sink, devices, 2, 1)
...
func TestRunRealtimeMultiCancellation(t *testing.T) {
	...
	runRealtimeMulti(ctx, newRand(), fieldFns, []float64{1.0}, 0, 0, 0, 0, sink, []string{"sensor-0"}, 100, 10)
...
func TestDropoutRateSkipsPoints(t *testing.T) {
	...
	// Full dropout: no points should be emitted.
	sink := &captureSink{}
	runBatch(seededRand(1), fns, sink, devices, time.Now().Unix(), 100, 1, 1.0)
	if len(sink.points) != 0 {
		t.Errorf("dropout-rate=1.0: expected 0 points, got %d", len(sink.points))
	}

	// Zero dropout: all points should be emitted.
	sink = &captureSink{}
	runBatch(seededRand(1), fns, sink, devices, time.Now().Unix(), 100, 1, 0.0)
	if len(sink.points) != 100 {
		t.Errorf("dropout-rate=0: expected 100 points, got %d", len(sink.points))
	}

	// 50% dropout: roughly half the points should be emitted.
	sink = &captureSink{}
	runBatch(seededRand(1), fns, sink, devices, time.Now().Unix(), 1000, 1, 0.5)
...
func TestDropoutRateMultiField(t *testing.T) {
	...
	// Full dropout.
	sink := &captureSink{}
	runBatchMulti(seededRand(1), fieldFns, scales, 0, 0, 0, 1.0, sink, devices, time.Now().Unix(), 100, 1)
	if len(sink.points) != 0 {
...
func TestSpreadProducesDifferentValues(t *testing.T) {
	...
	devices := []string{"sensor-0", "sensor-1"}
	runBatch(newRand(), fns, sink, devices, time.Now().Unix(), 1, 60, 0)
